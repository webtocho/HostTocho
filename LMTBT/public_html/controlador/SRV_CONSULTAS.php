...
define('SQL_PASSWORD', "tochoweb");
    ...
    switch ($_POST['tipo']) {
        case "login":
            $sql = sprintf("SELECT * FROM usuarios WHERE CORREO = '%s'  AND PASSWORD = '%s' ", $_POST['e_mail'], $_POST['password']);
            ...
            if ($info['ID_USUARIO']) {
                ...
                $db->setQuery("UPDATE usuarios SET ESTADO = 'CONECTADO' WHERE ID_USUARIO = " . $_SESSION['ID_USUARIO']);
                $db->ExecuteQuery();
                echo "ok";
            } else {
            ...
            break;
        case "logout":
            $db->setQuery("UPDATE usuarios SET ESTADO = 'DESCONECTADO' WHERE ID_USUARIO = " . $_SESSION['ID_USUARIO']);
            $db->ExecuteQuery();
        ...
        case "acceso_convocatoria":
            if (isset($_SESSION['ID_USUARIO']) && $_SESSION['TIPO_USUARIO'] == 'ADMINISTRADOR'){ echo "ok"; } else { echo "no"; }
            break;
        ...
        case "validar_registro_jugador":
            if (isset($_SESSION['ID_USUARIO']) && $_SESSION['TIPO_USUARIO'] == 'ADMINISTRADOR'){ echo "ok"; } else if(isset($_SESSION['ID_USUARIO']) && $_SESSION['TIPO_USUARIO'] == 'COACH'){ echo "no"; }
            break;          
            ...
            $estado = $_POST['estado'];
                ...
                if($categoria == "VARONIL" || $categoria == "FEMENIL" || $categoria == "HEAVY" || $categoria == "MIXTO" || $categoria == "RABBIT" || $categoria == "MAS DE 40"){
                    if(count($validar_fecha_cierre) == 3 && count($validar_fecha_inicio) == 3 && count($validar_fecha_fin) == 3){
                        if($validar_fecha_cierre[0] != "" && $validar_fecha_cierre[1] != "" && $validar_fecha_cierre[2] != ""
                            && $validar_fecha_inicio[0] != "" && $validar_fecha_inicio[1] != "" && $validar_fecha_inicio[2] != ""
                            && $validar_fecha_fin[0] != "" && $validar_fecha_fin[0] != "" && $validar_fecha_fin[2] != ""){
                            try{
        ...
        case  "roles_juegos_convocatoria_seleccionada":
            ...
            $roles_juego = "";
            foreach ($resultado as $key => $roles) {            
                    if(is_null($roles['GOLES_EQUIPO_1'])){ $goles_equipo_1 = 0;} else {$goles_equipo_1 = $roles['GOLES_EQUIPO_1'];}
                    if(is_null($roles['GOLES_EQUIPO_2'])){ $goles_equipo_2 = 0;} else {$goles_equipo_2 = $roles['GOLES_EQUIPO_2'];}
