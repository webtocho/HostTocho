<?php
	...
	$query="SELECT * FROM roles_juego WHERE FECHA >= CURDATE()+INTERVAL 1 DAY AND ID_EQUIPO_GANADOR=-1 LIMIT 5";
	...
	if($resultado && mysqli_num_rows($resultado)>0){
		...
		while ($row = mysqli_fetch_array($resultado) ) {
			...
			echo "<td class='ONE'>".$row['FECHA']." ".$row['HORA']."</td>";
			echo "<td clas='ONE'>".$name1['NOMBRE_EQUIPO']."</td>";
			echo "<td clas='ONE'>VS</td>";
			echo "<td clas='ONE'>".$name2['NOMBRE_EQUIPO']."</td>";
		}
		echo "</tbody>";
	}
